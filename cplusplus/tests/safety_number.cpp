...
TEST_CASE("Safety number", "[safety_number]") {
  ...
  auto a = Autograph::calculateSafetyNumber(aliceIdentityKey, bobIdentityKey);
  auto b = Autograph::calculateSafetyNumber(bobIdentityKey, aliceIdentityKey);

  REQUIRE_THAT(a, Catch::Matchers::Equals(safetyNumber));
  REQUIRE_THAT(b, Catch::Matchers::Equals(safetyNumber));
}
