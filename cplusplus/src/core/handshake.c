...
int autograph_handshake_derive_keys(unsigned char *our_secret_key,
...
void autograph_handshake_write_transcript(unsigned char *transcript,
...
int autograph_transcript(unsigned char *transcript,
  ...
  if (is_initiator == 1) {
    autograph_handshake_write_transcript(transcript, our_identity_key,
                                         their_identity_key, our_ephemeral_key,
                                         their_ephemeral_key);
  } else {
    autograph_handshake_write_transcript(transcript, their_identity_key,
                                         our_identity_key, their_ephemeral_key,
                                         our_ephemeral_key);
  }
  return 0;
}
...
int autograph_handshake_signature(
  ...
  int derive_result = autograph_handshake_derive_keys(
  ...
  int ciphertext_result =
      autograph_crypto_encrypt(message, our_secret_key, 0, our_signature, 64);
