...
void index_to_nonce(unsigned char *nonce, const unsigned int index) {
  nonce[AES_NONCE_SIZE - 4] = (index >> 24) & 0xFF;
  nonce[AES_NONCE_SIZE - 3] = (index >> 16) & 0xFF;
  nonce[AES_NONCE_SIZE - 2] = (index >> 8) & 0xFF;
  nonce[AES_NONCE_SIZE - 1] = index & 0xFF;
}
...
bool decrypt(unsigned char *plaintext, const unsigned char *key,
  ...
  sodium_memzero(nonce, AES_NONCE_SIZE);
  index_to_nonce(nonce, index);
  return crypto_aead_aes256gcm_decrypt(plaintext, NULL, NULL, ciphertext,
                                       ciphertext_size, NULL, 0, nonce,
                                       key) == 0;
}
...
bool encrypt(unsigned char *ciphertext, const unsigned char *key,
  ...
  sodium_memzero(nonce, AES_NONCE_SIZE);
  index_to_nonce(nonce, index);
  return crypto_aead_aes256gcm_encrypt(ciphertext, NULL, plaintext,
                                       plaintext_size, NULL, 0, NULL, nonce,
                                       key) == 0;
}
