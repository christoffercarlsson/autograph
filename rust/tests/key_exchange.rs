...
fn test_perform_key_exchange() {
    ...
    let mut alice_ephemeral_key_pair = KeyPair {
        private_key: vec![
    ...
    let mut bob_ephemeral_key_pair = KeyPair {
        private_key: vec![
    ...
    let mut alice = autograph.create_initiator(&alice_sign, &alice_identity_key_pair.public_key);
    let mut bob = autograph.create_responder(&bob_sign, &bob_identity_key_pair.public_key);
    let a = (alice.perform_key_exchange)(
        &mut alice_ephemeral_key_pair,
        &bob_identity_key_pair.public_key,
        &bob_ephemeral_key_pair.public_key,
    );
    let b = (bob.perform_key_exchange)(
    ...
    assert!(a.success);
    assert!(b.success);
    assert_eq!(a.key_exchange.handshake, alice_handshake);
    assert_eq!(b.key_exchange.handshake, bob_handshake);
}
