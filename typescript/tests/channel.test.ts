...
describe('Channel', () => {
  const aliceHandshake = Uint8Array.from([
    ...
    130, 225, 136, 185, 189, 45, 46, 67, 251, 154, 228, 146, 52, 116, 53, 88,
    24, 48, 162, 189, 105, 170, 0, 136, 236, 221, 184, 67, 110, 28, 244, 105,
    222, 198, 212, 57, 25, 136, 217, 177, 212, 163, 101, 16, 243, 152, 3, 12
  ])

  const bobHandshake = Uint8Array.from([
    22, 51, 47, 208, 198, 143, 141, 242, 199, 185, 82, 142, 190, 105, 55, 152,
    145, 185, 67, 35, 122, 253, 201, 23, 74, 40, 110, 217, 60, 198, 123, 216,
    195, 74, 74, 185, 65, 215, 2, 151, 214, 117, 91, 122, 16, 145, 253, 88, 26,
    50, 135, 226, 45, 126, 125, 22, 88, 214, 178, 147, 69, 72, 143, 3
  ])

  const aliceMessage = Uint8Array.from([
    42, 72, 111, 245, 73, 128, 9, 3, 155, 238, 25, 61, 200, 215, 73, 154, 133,
    58, 84, 72, 232, 161, 24, 148, 226, 254, 211, 47, 90, 167, 86, 120
  ])

  const bobMessage = Uint8Array.from([
    189, 114, 27, 132, 42, 6, 4, 7, 56, 49, 37, 113, 187, 43, 159, 60, 220, 53,
    7, 211, 224, 204, 25, 49, 234, 20, 145, 124, 171, 220, 49, 182
  ])
  ...
  const safetyNumber = Uint8Array.from([
    0, 1, 33, 28, 0, 0, 133, 8, 0, 0, 122, 169, 0, 1, 95, 147, 0, 1, 10, 56, 0,
    0, 92, 202, 0, 1, 129, 249, 0, 0, 37, 3, 0, 0, 89, 52, 0, 0, 133, 56, 0, 0,
    255, 89, 0, 0, 129, 161, 0, 1, 132, 70, 0, 0, 169, 215, 0, 0, 29, 42, 0, 0,
    153, 44
  ])
  ...
  it('should calculate safety numbers correctly', () => {
    const aliceId = Uint8Array.from([97, 108, 105, 99, 101])
    const bobId = Uint8Array.from([98, 111, 98])
    const aliceSafetyNumber = a.authenticate(aliceId, bobId)
    const bobSafetyNumber = b.authenticate(bobId, aliceId)
